...
from wrs_semantics.srv import Recognize_drink, Recognize_drinkRequest

class hsr_AnalyseObjectBringMeState(EventState):
    '''
    ...
    def __init__(self, service_name='/wrs_semantics/tidyup_recognizeObject_bringme'):
        super(hsr_AnalyseObjectBringMeState,self).__init__(input_keys=['command'], output_keys=['object_name'], outcomes=['succeeded', 'failed'])

        self._service_name = service_name

        self._get_object_location_srv = ProxyServiceCaller({self._service_name : Recognize_drink})
    ...
    def on_enter(self, userdata):
        ...
        if type(userdata.command) is str:
            req = Recognize_drinkRequest(userdata.command)
        else :
            req = Recognize_drinkRequest(userdata.command.message)
        ...
        try:
            ...
            rospy.loginfo(self._srv_result.object_name)
